...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        if cmd_listener.get_msg() == "HIGH":
            ...
            camera.save_img("/home/ece414e/Documents/led_on/photo{}.jpg".format(i))
            while cmd_listener.get_msg() == "HIGH":
                pass
        if cmd_listener.get_msg() == "LOW":
            time.sleep(0.45)
            led_off = camera.get_img()
            camera.save_img("/home/ece414e/Documents/led_off/photo{}.jpg".format(i))
            while cmd_listener.get_msg() == "LOW":
                pass
        if (led_on is not  None and led_off is not None):
            diff = cv2.absdiff(led_on, led_off)
            cv2.imwrite("/home/ece414e/Documents/led_diff/photo{}.jpg".format(i), diff)
            print(i)
            i+=1
        rate.sleep()
