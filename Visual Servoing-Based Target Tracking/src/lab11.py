...
NS = 25                     # search size
print("type ", type(NT))
CEN_wt = 0.001 # Center Steering weight
DIR_wt = 0.006 # Direction Steering weight
    ...
    while not rospy.is_shutdown():
      while ipr == 0:
        ...
        if (led_on is None or led_off is None):
            diff = np.zeros([480,640], dtype=np.int32)
            if(led_on == None):
                print("led_on ", led_on)
            if(led_off == None):
        ...
        if ipr == 1:
            ...
            print("ipr = ", ipr)
            centr = ( Clust[2,1] + Clust[0,1] )/2. #center of front LEDs
            direc = 320 - centr #driving direction error
            offCent = centr - Clust[1,1] #rear LED error
            print("direc, centr, offCent = ", direc, centr, offCent)
        ...
        if ipr == 4:
             print('i should be moving now')
             motor.move_with_coor(.07, 0.)
        if ipr > 4:
            if(dist < 18.):
                motor.move_with_coor(0., 0.)
                break
            trn = -offCent*CEN_wt + direc * DIR_wt
            print("****************************************** turn *********************************", trn)
            if(trn > .05):
